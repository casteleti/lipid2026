...
export default function TecnologiasPage() {
  ...
  useEffect(() => {
    api
      .get<Paginated<Technology>>('/technologies?take=100')
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Tecnologias</h1>
      ...
      <Card>
        {loading ? (
          ...
          <p className="text-gray-500 py-8 text-center">Nenhuma tecnologia cadastrada ainda.</p>
        ) : (
              ...
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm">
                    <Link href={`/tecnologias/${item.id}`} className="text-primary-600 hover:underline">
                      Editar
                    </Link>
                  </td>
                </tr>
