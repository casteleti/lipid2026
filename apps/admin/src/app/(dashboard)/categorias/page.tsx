...
export default function CategoriasPage() {
  ...
  useEffect(() => {
    api
      .get<Paginated<Category>>('/categories?take=100')
    ...
    <div>
      ...
      <Card>
        {loading ? (
          ...
          <p className="text-gray-500 py-8 text-center">Nenhuma categoria cadastrada ainda.</p>
        ) : (
              ...
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm">
                    <Link href={`/categorias/${item.id}`} className="text-primary-600 hover:underline">
                      Editar
                    </Link>
                  </td>
                </tr>
